    ...
    const fetchStates = async () => {
      try {
        const res = await axios.get(
          `${API_BASE_URL}/api/location/states`
        );
        setStates(res.data);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success || res.status === 201) {
        showSuccess("Registration Successful!", "Your account has been created successfully. Please log in.");
        setTimeout(() => {
          router.push("/login");
        }, 1500);
      } else {
        showError("Registration Failed", res.data.message || "Registration failed");
      }
    } catch (err: any) {
      showError("Registration Error", err.response?.data?.message || "Server error");
    } finally {
      setLoading(false);
    }
  };
