...
//   // ⭐ Fetch operation mode from backend
...
//       if (!res.ok) throw new Error(data.error || "Failed to load operation mode");
...
//   // ⭐ Update operation mode to backend
...
//         mode === "wifi" ? "WiFi mode enabled" : "Manual mode enabled"
//       );
//     } catch (err: any) {
...
//         // ⭐ Fetch backend operation mode FIRST
...
//         const cropsList: CropPreset[] = Array.isArray(cropsJson?.crops)
//           ? cropsJson.crops
//           : cropsJson;
...
//             const match = cropsList.find((c) => c.crop_name === latest.crop_name);
...
//         }
//       } catch (e: any) {
//         showError("Loading Error", e.message || "Failed to load controls");
//       } finally {
//         setLoading(false);
//       }
//     };
...
//         const cleanedStages = manualFanStages.map((v) =>
...
//           )
//         ) {
//           body.fan_stages = cleanedStages.map((v) =>
...
//       );
//     } catch (e: any) {
//       showError("Configuration Error", e.message || "Failed to submit configuration");
//     } finally {
//       setSubmitting(false);
//     }
//   };
...
//                 onClick={() => router.push(`/user-dashboard/${deviceName}`)}
//               >
//                 ← Device Data
//               </button>
//               <button
...
//             {/* Right Column - Status then Fan Control */}
//             <div className="space-y-6">
//               {/* Status Card */}
//               <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
//                 <div className="flex items-center gap-3 mb-6">
...
//                 <div className="space-y-6">
//                   {/* Preset fan stages or manual input */}
...
//                   {/* Circular fan speed removed as requested */}
//                 </div>
//               </div>
...
>
  ...
  {presetOptions.map((p) => (
    <option
      key={p._id}
      value={p._id}
      disabled={p.expired}          // ⬅️ cannot select expired crop
      className={p.expired ? "text-red-400" : "text-black"} // ⬅️ visually different
    >
