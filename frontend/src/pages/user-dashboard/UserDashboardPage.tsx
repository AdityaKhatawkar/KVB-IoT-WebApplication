        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {devices.map((device) => {
            ...
            return (
              ...
              >
                {/* Header Row: Status Left + Device Name Right */}
                <div className="flex items-center justify-between mb-6">
                  ...
                  <h3 className="text-2xl font-extrabold text-slate-800 tracking-wide">
                    {device.device_name}
                  </h3>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-100">
                    ...
                    <div className="text-xl font-bold mt-1">{daysRemaining}</div>
                  </div>

                  <div className="bg-green-50 rounded-lg p-4 border border-green-100">
                    <span className="text-xs font-medium text-green-600">Preset</span>
                    <div className="text-xl font-bold mt-1">{device.preset || "N/A"}</div>
                  </div>

                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                    <span className="text-xs font-medium text-blue-600">Start Date</span>
                    <div className="mt-1">{start ? start.toLocaleDateString("en-IN") : "N/A"}</div>
                  </div>

                  <div className="bg-orange-50 rounded-lg p-4 border border-orange-100">
                    <span className="text-xs font-medium text-orange-600">Expiry</span>
                    <div className="mt-1">{end ? end.toLocaleDateString("en-IN") : "N/A"}</div>
                  </div>
                </div>
